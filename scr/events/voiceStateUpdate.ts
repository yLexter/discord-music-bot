...
export default {
  ...
  execute: async (client: any, oldState: VoiceState, newState: VoiceState) => {
    ...
    if (
      oldState.channelId != channelBot.id &&
      !oldState.channelId &&
      newState.channelId
    )
      return;
    ...
    function verifyBotAlone() {
      const allMembers = [...channelBot.members.values()].filter(
        (x) => x.user.id == client.user.id || !x.user.bot
      );

      if (
        allMembers.length != 1 &&
        allMembers.find((x) => x.id == client.user.id)
      )
        return;
      ...
      setTimeout(() => {
        if (!time.has(guild.id)) return;

        const queue = client.queues.get(guild.id);

        if (!queue) return time.delete(guild.id);

        const channelId = queue.getConnection()?.joinConfig?.channelId;
        const channel = guild.channels.cache.get(channelId);

        if (channel && [...(channel as any).members.values()].length > 1)
