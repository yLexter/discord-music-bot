...
class SlashCommand {
  ...
  async executeCommand() {
    ...
    if (!guild.members.me.permissions.has("ADMINISTRATOR"))
      return client.embedError(
        interaction,
        "Preciso da permissão de administrator para executar os comandos"
      );
    ...
    if (cdCommand) {
      const time =
        defaultTimeCD - Math.floor(Date.now() / 1000 - cdCommand) || "??";
      return client.embedError(
        interaction,
        `Este comando está em cooldown, aguarde \`${time}s.\``
      );
    }
    ...
    function music() {
      ...
      if (queue?.player._state.status == AudioPlayerStatus.Idle)
        return client.embedError(
          interaction,
          "Comandos não podem ser usado no momento de trocas de músicas"
        );
    ...
    function executeCmd() {
      ...
      return client.commands
        ...
        .catch((e: any) => client.embedError(interaction, e));
    }
  }
}
